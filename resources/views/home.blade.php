                    ...
                    <a href="{{ route('preview', $popularBook) }}" class="col text-dark text-decoration-none">
                        <div class="card p-2" style="height: 500px;">
                            <img src="{{ isset($popularBook->cover) ? asset('storage/' . $popularBook->cover) : asset('storage/placeholder.png') }}"
                                 alt="{{ $popularBook->title }}" class="card-img-top" style="height: 90%; object-fit: cover;">
                            <div class="card-body text-center p-2 d-flex flex-column justify-content-between" style="height: 20%;">
                                <h3 class="card-text fs-6 fw-bold mb-1">{{ $popularBook->title }}</h3>
                                <span class="fs-7">Dipinjam 
                                    <span class="fw-bold text-decoration-underline">{{ $popularBook->borrows_count }}</span> kali
                                </span>
                            </div>
                        </div>
                    </a>
                    ...
                    <a href="{{ route('preview', $newestBook) }}" class="col text-dark text-decoration-none">
                        <div class="card p-2" style="height: 500px;">
                            <img src="{{ isset($newestBook->cover) ? asset('storage/' . $newestBook->cover) : asset('storage/placeholder.png') }}"
                                 alt="{{ $newestBook->title }}" class="card-img-top" style="height: 90%; object-fit: cover;">
                            <div class="card-body text-center p-2 d-flex flex-column justify-content-between" style="height: 20%;">
                                <h3 class="card-text fs-6 fw-bold mb-1">{{ $newestBook->title }}</h3>
                                <span class="fs-7">Terbit 
